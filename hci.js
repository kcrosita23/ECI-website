...
function sendEmail(event) {
    ...
    if (contact === "" || firstName === "" || lastName === "" || company === "" ||
        position === "" || phone === "" || mobile === "" || email === "" || message === "") {
        Swal.fire({
            title: "Error!",
    ...
    if (!namePattern.test(firstName) || !namePattern.test(lastName)) {
        Swal.fire("Invalid Name", "First and last names must contain only letters and be 2–30 characters long.", "error");
        return;
    }


    if (company.length > 50 || position.length > 50) {
        Swal.fire("Too Long", "Company or position exceeds max allowed length.", "error");
        return;
    }
    ...
    if (!phonePattern.test(phone)) {
        Swal.fire("Invalid Phone", "Enter a valid phone number or 'None'.", "error");
        return;
    }

    if (!mobilePattern.test(mobile)) {
        Swal.fire("Invalid Mobile", "Enter a valid mobile number or 'None'.", "error");
        return;
    }


    let params = {
        contact, firstName, lastName, company, position, phone, mobile, email, message
    };
